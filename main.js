 ...
 * 封面由 CI 预生成到 covers/，部署后完全从 GitHub Pages 静态资源加载。
 * 未成功生成的封面会自动回退为彩色节点，不影响图谱使用。
 *
 * 构建：npm run build（esbuild → main.bundle.js），需先 npm install。
 * 运行：python -m http.server 1119，然后访问 http://127.0.0.1:1119/
 */
...
// ==================== 静态封面 ====================
//
// GitHub Pages 没有后端代理，因此封面由 CI 在部署前下载到 covers/。
// manifest 只列出实际成功下载的节点；没有封面的节点使用普通圆形渲染。
async function loadCoverManifest() {
  try {
    const res = await fetch(COVER_MANIFEST_URL, { cache: "no-store" });
...
function main() {
  ...
  async function boot() {
    ...
    setProgress(20, coverKeys.size ? "构建图结构……" : "未找到封面，使用纯色节点……",
      coverKeys.size ? (coverKeys.size + " 张封面已预生成") : "CI 可生成静态封面");
